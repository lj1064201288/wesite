...
class SideBar(models.Model):
    ...
    def content_html(self):
        ...
        if self.display_type == self.DISPLAY_HTML:
            result = mistune.markdown(self.content)
            ...
            context = {
                'comments': Comment.objects.filter(status=Comment.STATUS_NORMAL).order_by('-id')[:10],
            }
            result = render_to_string('config/blocks/sidebar_comments.html', context)
        elif self.display_type == self.DISPLAY_HOT:
            content = {
                'hots': Post.hot_posts()[:5]
            }
            result = render_to_string('config/blocks/sidebar_hots.html', content)
    ...
    @classmethod
    def get_all(cls):
        # print(cls.objects.filter(status=cls.STATUS_SHOW))
